...
class DashboardController {
    ...
    renderChart() {
        ...
        if (validatedConfig.is_multi_series && validatedConfig.group_by_column) {
            ...
            traces = [{
                ...
                type: validatedConfig.chart_type === 'grouped_bar' ? 'bar' : validatedConfig.chart_type || 'bar',
    ...
    processChartData(config) {
        ...
        if (yAxis === 'count') {
            ...
            if (this.isYearMonthColumn(xAxis)) {
                const dataPoints = this.rawData.map(row => ({
    ...
    processMultiSeriesData(config) {
        ...
        if (groupNames.length > maxSeries && config.show_others_group) {
            ...
            const groupTotals = groupNames.map(groupName => {
    ...
    isYearMonthColumn(columnName) {
        ...
        return lowerName.includes('달력_연도_월') || lowerName.includes('연도_월') || lowerName.includes('year_month');
    }
    ...
    isYearColumn(columnName) {
        ...
        return lowerName.includes('year') || lowerName === 'year_id' || lowerName === 'yearid' ||
               lowerName.includes('month') || lowerName === 'month_id' || lowerName === 'monthid' ||
               lowerName.includes('년도') || lowerName.includes('연도');
    }
...
class MultiChartDashboardController extends DashboardController {
    ...
    processMultiSeriesDataForChart(config, data) {
        ...
        console.log(`Processing multi-series chart: xAxis=${xAxis}, yAxis=${yAxis}, groupBy=${groupBy}`);
        ...
        const layout = {
            ...
            xaxis: {
                title: this.getAxisTitle(config.x_axis),
                type: this.getXAxisType(data.map(row => row[xAxis]), config.x_axis)
            },
            yaxis: {
                title: this.getAxisTitle(config.y_axis),
                tickformat: this.isLargeNumber(data.map(row => parseFloat(row[yAxis]) || 0)) ? ',.0s' : '.2f'
            },
            margin: { t: 40, r: 20, b: 40, l: 80 },
...
document.addEventListener('DOMContentLoaded', function() {
    try {
        ...
        const chartConfig = {{ (chart_config or {}) | tojson | safe }};
        ...
        // 다중 차트 vs 단일 차트 결정
        const isMultiChart = {{ 'true' if dashboard.is_multi_chart else 'false' }};
        const chartConfigs = {{ (chart_configs | tojson | safe) if dashboard.is_multi_chart else '[' + (chart_config | tojson | safe) + ']' }};

        if (isMultiChart) {
            window.dashboardController = new MultiChartDashboardController(dashboardId, chartConfigs, true);
        } else {
            window.dashboardController = new DashboardController(dashboardId, chartConfigs[0] || chartConfig);
        }
    } catch (error) {
