...
class MultiChartDashboardController extends DashboardController {
  ...
  renderAllCharts() {
    this.chartConfigs.forEach((config, index) => {
      // 차트 헤더 업데이트
      const chartHeader = document.querySelector(`.dashboard__content__chart_item[data-chart-index="${index}"] .dashboard__content__chart_header h6`);
      if (chartHeader) {
...
document.addEventListener("DOMContentLoaded", function () {
  try {
    ...
    // Multi-chart 레이아웃 표시
    const multiChartContainer = document.querySelector('.dashboard__content__multi_chart_container');
